...
#include <stdarg.h>
 ...
 /**
  * @brief Check CUDA error and print error message
  */
 ...
 /**
  * @brief CUDA kernel to process pattern matching with existing forward simulation data
  * @param outArray Array containing spot positions from forward simulation
  * @param image The detector image
  * @param nrPxY Number of pixels in Y direction
  * @param nrPxTotal Total number of pixels
  * @param orientationsPerThread Number of orientations per thread
  * @param maxNumSpots Maximum number of spots to consider
  * @param minNumSpots Minimum number of spots for a match
  * @param minIntensity Minimum intensity for a match
  * @param matchedScores Output array for match scores
  */
 __global__ void laue_match_kernel(
     const uint16_t *outArray,
     const double *image,
     int nrPxY,
     int nrPxTotal,
     int orientationsPerThread,
     int maxNumSpots,
     int minNumSpots,
     double minIntensity,
     ...
     for (int orientNr = startOrientNr; orientNr < startOrientNr + orientationsPerThread; orientNr++) {
         int spotCount = 0;
         int matchCount = 0;
 ...
 /**
  * @brief Count non-zero values in an array
  */
 ...
 int laue_init_gpu(void) {
     ...
     if (error != cudaSuccess || deviceCount == 0) {
         laue_log(0, "ERROR: No CUDA devices available");
         return LAUE_ERROR_GPU;
     }
 ...
 int laue_init(void) {
     ...
     // Initialize GPU
     int ret = laue_init_gpu();
     if (ret != LAUE_SUCCESS) {
         laue_log(0, "WARNING: GPU initialization failed, falling back to CPU");
     }
 ...
 int laue_perform_matching_gpu(
     ...
     if (!gpu_initialized) {
         laue_log(0, "WARNING: GPU not initialized, using CPU implementation");
         return LAUE_ERROR_GPU;
     }
     ...
     // Check if forward simulation data exists - if so, use GPU
     if (doForwardSimulation == 0) {
         // Setup CUDA memory
         double *d_image = NULL;
         double *d_matchedScores = NULL;
         uint16_t *d_outArray = NULL;
         int *d_numResults = NULL;
         int h_numResults = 0;
         
         // Allocate device memory
         laue_log(1, "Allocating GPU memory for pattern matching");
         
         // Allocate memory for the image
         CHECK_CUDA_ERROR(cudaMalloc((void**)&d_image, nrPxTotal * sizeof(double)));
         CHECK_CUDA_ERROR(cudaMemcpy(d_image, image, nrPxTotal * sizeof(double), cudaMemcpyHostToDevice));
         
         // Allocate memory for matched scores
         CHECK_CUDA_ERROR(cudaMalloc((void**)&d_matchedScores, numOrientations * sizeof(double)));
         CHECK_CUDA_ERROR(cudaMemset(d_matchedScores, 0, numOrientations * sizeof(double)));
         
         // Allocate memory for result counter
         CHECK_CUDA_ERROR(cudaMalloc((void**)&d_numResults, sizeof(int)));
         CHECK_CUDA_ERROR(cudaMemset(d_numResults, 0, sizeof(int)));
         
         // Determine GPU kernel parameters
         int threadsPerBlock = 256;
         int blocksPerGrid = (numThreads + threadsPerBlock - 1) / threadsPerBlock;
         
         // Process forward simulation data in chunks
         size_t arraySize = orientationsPerThread * (1 + 2 * maxNumSpots);
         size_t chunkSize = 10 * arraySize; // Process 10 threads at a time
         
         // Allocate device memory for forward simulation data
         CHECK_CUDA_ERROR(cudaMalloc((void**)&d_outArray, chunkSize * sizeof(uint16_t)));
         
         laue_log(1, "Running GPU pattern matching with %d blocks, %d threads per block", 
                  blocksPerGrid, threadsPerBlock);
         
         // Process in chunks to avoid excessive memory usage
         for (int chunkStart = 0; chunkStart < numThreads; chunkStart += 10) {
             int chunkEnd = chunkStart + 10;
             if (chunkEnd > numThreads) chunkEnd = numThreads;
             int chunkThreads = chunkEnd - chunkStart;
             
             // Read chunk of forward simulation data
             uint16_t *chunkOutArray = (uint16_t *)malloc(chunkSize * sizeof(uint16_t));
             if (chunkOutArray == NULL) {
                 laue_log(0, "ERROR: Failed to allocate memory for forward simulation chunk");
                 cudaFree(d_image);
             ...
             size_t chunkOffset = chunkStart * arraySize * sizeof(uint16_t);
             ...
             CHECK_CUDA_ERROR(cudaMemcpy(d_outArray, chunkOutArray, chunkSize * sizeof(uint16_t), cudaMemcpyHostToDevice));
             
             // Launch kernel for this chunk
             laue_match_kernel<<<chunkThreads, threadsPerBlock>>>(
                 d_outArray,
                 d_image,
                 nrPxY,
                 nrPxTotal,
                 orientationsPerThread,
                 maxNumSpots,
                 config->minNumSpots,
                 config->minIntensity,
                 d_matchedScores + chunkStart * orientationsPerThread
             );
         ...
         CHECK_CUDA_ERROR(cudaMemcpy(matchedScores, d_matchedScores, numOrientations * sizeof(double), cudaMemcpyDeviceToHost));
         
         // Count number of valid results
         int *d_countBuffer;
         CHECK_CUDA_ERROR(cudaMalloc((void**)&d_countBuffer, sizeof(int)));
         CHECK_CUDA_ERROR(cudaMemset(d_countBuffer, 0, sizeof(int)));
         
         // Configure kernel for counting
         threadsPerBlock = 256;
         blocksPerGrid = (numOrientations + threadsPerBlock - 1) / threadsPerBlock;
         
         // Count non-zero scores
         count_nonzero_kernel<<<blocksPerGrid, threadsPerBlock>>>(
             d_matchedScores, 
             numOrientations, 
             d_countBuffer
         );
         ...
         // Get result count
         CHECK_CUDA_ERROR(cudaMemcpy(&h_numResults, d_countBuffer, sizeof(int), cudaMemcpyDeviceToHost));
         ...
         cudaFree(d_numResults);
         cudaFree(d_outArray);
         cudaFree(d_countBuffer);
         
         laue_log(1, "GPU pattern matching found %d initial matches", h_numResults);
         
         time_checkpoint = omp_get_wtime();
         laue_log(1, "GPU pattern matching completed in %.2f seconds", time_checkpoint - start_time);
     } else {
         // Forward simulation is required, use CPU implementation
         laue_log(1, "Forward simulation required, falling back to CPU implementation");
         ...
         #pragma omp parallel num_threads(numThreads) reduction(+:numResults) shared(anyThreadError)
         {
             ...
             if (!threadError) {
                 ...
                 // Write forward simulation data
                 ret = file_write_forward_simulation(
                     config->forwardSimulationFile,
                     outArray,
                     arraySize,
                     arrayOffset
         ...
         laue_log(1, "Initial matches found: %d", h_numResults);
     }
