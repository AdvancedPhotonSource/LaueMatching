...
All paths come from the environment so this runs against any scan:
    ...
    LAUE_PARAMS_<PHASE>  the phase's params file (its space group sets the symmetry)
...
W=os.environ.get("LAUE_WORK") or sys.exit(
    "LAUE_WORK is not set (output root; peel_map/ and figures/ are written under it).\n" + __doc__)
DATA=os.environ.get("LAUE_SCAN_DATA") or sys.exit(
...
if __name__ == "__main__":
    ...
    # cluster into grains (subsample-safe greedy). Operators follow the phase's space
    # group (laue_material): the old "alpha -> hex, anything else -> cubic" rule gave a
    # hex phase with any other name 24 cubic operators. (_ph is loaded at the top so a
    # missing params file fails before the read pass, not after it.)
