// gcc LaueMatchingCPU.c -o LaueMatchingCPU -I{NLOPT/include} -L{NLOPT/lib} -lnlopt -fopenmp -fPIC -g -ldl -lm -fgnu89-inline -O3 -w -Wall -fopt-info
//
...
static inline void MatrixMultF33(double m[3][3],double n[3][3],double res[3][3])
{
	int r;
	for (r=0; r<3; r++) {
		res[r][0] = m[r][0]*n[0][0] +
...
static inline double zeroOut(double val){
...
static inline void calcRecipArray(double Lat[6], int SpaceGroup, double recip[3][3]){
...
static inline double calcOverlap(double *image, double euler[3], int *hkls, int nhkls, int nrPxX, int nrPxY,
...
inline static double problem_function(unsigned n, const double *x, double *grad, void* f_data_supplied)
{
	int i, j;
...
static inline double FitOrientation(double *image, double euler[3], int *hkls, int nhkls, int nrPxX, int nrPxY,
	double recip[3][3], double *outArrThis, int maxNrSpots, double rotTranspose[3][3], double pArr[3], double pxX, 
	double pxY, double Elo, double Ehi, double tol, double latc[6], double eulerFit[3], double latCUpd[6], double *minVal)
{
	int i,j;
	unsigned n;
	if (doCrystalFit==0){
...
int main(int argc, char *argv[])
{
	...
	double *outArrThisFit, mv =0, tol= 3*deg2rad;
	...
	for (global_iterator=0;global_iterator<nrOrients;global_iterator++){
		...
		for (k=0;k<3;k++){
			eulerFit[k] = eulerBest[k];
			for (l=0;l<3;l++){
				orientBest[k][l] = orients[bestSol*9+k*3+l];
			}
		}
		...
		FitOrientation(image,eulerBest,hkls,nhkls,nrPxX,nrPxY,recip,outArrThisFit,maxNrSpots,
			rotTranspose,pArr,pxX,pxY,Elo,Ehi,tol,LatticeParameter,eulerFit,latCFit,&mv);
		doCrystalFit = 1;
		for (k=0;k<3;k++) eulerBest[k] = eulerFit[k];
		FitOrientation(image,eulerBest,hkls,nhkls,nrPxX,nrPxY,recip,outArrThisFit,maxNrSpots,
			rotTranspose,pArr,pxX,pxY,Elo,Ehi,tol,LatticeParameter,eulerFit,latCFit,&mv);
		Euler2OrientMat(eulerFit,orientFit);
		OrientMat2Quat33(orientBest,quat1);
		OrientMat2Quat33(orientFit,quat2);
		if (nrSps<minNrSpots){
			continue;
		}
		misoAngle = GetMisOrientation(quat1,quat2,sg_num);
		fprintf(outF,"%d\t%d\t",unique+1,doneArr[global_iterator]);
		fprintf(outF,"%-13.4lf\t",(mv/nrSps)*(mv/nrSps));
		fprintf(outF,"%-13.4lf\t",nrSps*(mv/nrSps)*(mv/nrSps));
		saveExtraInfo = unique+1;
		double intMatch;
		calcRecipArray(latCFit,sg_num,recipFit);
		intMatch = calcOverlap(image,eulerFit,hkls,nhkls,nrPxX,nrPxY,recipFit,outArrThisFit,
			maxNrSpots,rotTranspose,pArr,pxX,pxY,Elo,Ehi);
		...
		fprintf(outF,"%-13.4lf\t%-13.7lf\t%d\n",matchedArr[bestSol],misoAngle,bestSol);
		unique++;
	}
	fclose(ExtraInfo);
	fclose(outF);
