...
CFLAGSNLOPT=-I$(current_dir)/LIBS/NLOPT/include -L$(current_dir)/LIBS/NLOPT/lib \
	-L$(current_dir)/LIBS/NLOPT/lib64 -lnlopt
NCFLAGS=-lgomp -O3 -G -g -w -gencode=arch=compute_70,code=sm_70 \
	-gencode=arch=compute_86,code=sm_86 \
	-gencode=arch=compute_90,code=sm_90 \
	-gencode=arch=compute_80,code=sm_80 \
	-lm -rdc=true -Xcompiler=-fopenmp
SRC=src/
BIN=bin/
